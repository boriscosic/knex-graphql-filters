    ...
    // or
      ...
      map(value, (opValue, op) => {
        switch (op) {
          ...
          case "json_contains_raw": {
            if (or) {
              query.orWhereRaw(`${key} @> ${opValue}`);
            } else {
              query.whereRaw(`${key} @> ${opValue}`);
            }
            break;
          }

          case "json_contains_any_raw": {
            if (or) {
              query.orWhereRaw(`${key} && ${opValue}`);
            } else {
              query.whereRaw(`${key} && ${opValue}`);
          ...
          case "overlaps": {
            if (or) {
              query.orWhereRaw(`greatest(??, '${opValue.start}'::timestamptz) <= least(??, '${opValue.end}'::timestamptz)`, [key, opValue.with]);
            } else {
              query.whereRaw(`greatest(??, '${opValue.start}'::timestamptz) <= least(??, '${opValue.end}'::timestamptz)`, [key, opValue.with]);
            }
            break;
          }
          ...
          case "date_range_overlaps_date_range": {
            if (or) {
              query.orWhereRaw("('??', date '??' + INTERVAL '1' DAY) overlaps (??, ??)",
              [`'${opValue.start}'`, `'${opValue.end}'`, key, opValue.with]
              );
            } else {
              query.whereRaw("('??', date '??' + INTERVAL '1' DAY) overlaps (??, ??)",
                [opValue.start, opValue.end, key, opValue.with]
              );
            }
            break;
          }
          case "array_contains_string": {
            if (or) {
              query.orWhereRaw("? = ALL(string_to_array(" + key + ", ';')::varchar[])", [opValue]);
            } else {
              query.whereRaw("? = ALL(string_to_array(" + key + ", ';')::varchar[])", [opValue]);
            }
            break;
          }
          case "array_not_contains": {
            if (or) {
              query.orWhereRaw("not(? = ANY(" + key + "))", [opValue]);
            } else {
              query.whereRaw("not(? = ANY(" + key + "))", [opValue]);
            }
            break;
          }
          case "array_contains": {
            if (or) {
              query.orWhereRaw("? = ANY(" + key + ")", [opValue]);
            } else {
              query.whereRaw("? = ANY(" + key + ")", [opValue]);
            }
            break;
          }
